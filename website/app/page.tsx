...
const modes: Record<Mode, { label: string; short: string; rule: string; example: string; fields: string[]; values: [string, string, string] }> = {
  ...
  goal: {
    ...
    fields: ["완료 조건", "확인 방법", "허용 범위"],
    values: ["30일 인스타그램 콘텐츠 캘린더 초안 완성", "주제·게시일·핵심 문구·이미지 방향이 모두 채워진 표", "브랜드 톤과 제공된 상품 정보만 사용"],
  },
  eval: {
    label: "기존 프롬프트 개선",
    short: "실패 분석 · 회귀 검증",
...
export default function Home() {
  ...
  const practicePrompt = useMemo(() => {
    ...
    if (practiceMode === "goal") return `/goal ${goal}, ${who}로 완료를 확인하세요. ${output} 범위만 사용하세요. 반복마다 실제 상태를 확인하고 실패하면 전략을 바꾸세요. 사용자가 지정한 최대 턴수나 시간 한도에 도달하면 남은 작업과 증거를 보고하세요. 완료 확인 기준과 결과를 대화에 남기세요. /goal은 권한을 자동 승인하지 않으므로 권한 설정은 별도로 확인하세요. 보안·배포·파괴적 변경처럼 위험이 큰 작업은 완료 전 회귀·제약·비밀 노출을 검토하세요.`;
